...
def check_source_manifest(skill_path: Path, content: str) -> tuple[bool, str]:
    """要求来源元数据存在于独立、可浏览的清单文件。"""
    manifest = skill_path.parent / "references" / "sources.md"
    linked = bool(re.search(r"references/sources\.md", content))
    if not manifest.exists():
        return False, "❌ 缺少 references/sources.md"
    if not linked:
        return False, "❌ SKILL.md 未链接 references/sources.md"
    return True, "独立来源清单存在且已链接 ✅"
...
def main():
    ...
    checks = [
        ...
        ("独立来源清单", lambda text: check_source_manifest(skill_path, text)),
    ]
