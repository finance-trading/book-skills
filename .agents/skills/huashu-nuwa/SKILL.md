...
**完成检查**（自动执行）：
- [ ] 目录已创建
- [ ] `references/sources.md` 已创建，作为独立、可浏览的来源清单
...
- 每条研究结论在研究文件中使用稳定的来源编号（如 `S01`），并注明一手/二手/推测、URL或本地素材路径、访问/出版日期和可信度。
- 所有来源元数据必须在 Phase 1.5 汇总到 `references/sources.md`；它是来源的唯一索引，按「一手来源」「独立二手来源」「利益相关方来源」「不可核验/待补充」分组。不要只把来源散落在六个研究文件或最终 SKILL.md 中。
- `SKILL.md` 的来源部分只保留到 `references/sources.md` 的链接和一句来源边界说明；详细书目、URL、状态和引用定位放在独立文件中。
...
- 每条事实在该文件中标注来源编号（`S01`、`S02`……）；不要只写裸 URL。
- 注明信息来源和可信度（一手>二手>推测）
...
所有 Agent 完成后，主 Agent 必须创建或更新 `references/sources.md`，将研究文件中的来源编号去重并汇总。每行至少包含：`ID`、标题/作者、URL或本地路径、来源类型、出版/访问日期、支持的研究维度、可信度、可核验状态。来源清单完成前，不进入 Phase 2 提炼。
...
| 调研来源 | 只链接 `references/sources.md`；详细书目、URL、状态和引用定位不内嵌在 SKILL.md |
...
| 一手来源占比 | >50% | 主要依赖二手转述 |
| 独立来源清单 | `references/sources.md` 存在且覆盖所有研究来源编号 | 来源散落、无法追溯 |
